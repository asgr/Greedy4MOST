FibreAESOP=function(RA_data, Dec_data, RA_AESOP=0, Dec_AESOP=0, pri_data=9, res_data='lo', assign_AESOP=TRUE, avoid=0.9){
  ...
  for(pri in pri_data_list){
    reftemp=ref[pri_data==pri & res_data %in% 'lo']
    if(length(reftemp)==0){break}
    match_fib=coordmatch(AESOP_pos$sph[select_AESOP_lo,], cbind(RA_data[reftemp], Dec_data[reftemp]), rad=AESOP_fibres$patrol_asec[select_AESOP_lo])
